...
# NVDR_ROCM_C10_CMAKE_MACROS_STUB
...
# PyTorch's ROCm wheels DO ship the c10/cuda headers, but NOT
# c10/cuda/impl/cuda_cmake_macros.h: that file is generated by CMake and only
# exists in CUDA builds. Result:
#   fatal error: 'c10/cuda/impl/cuda_cmake_macros.h' file not found
# The header is only a configure-time stub. On Linux the sole macro it can
# define is C10_CUDA_BUILD_SHARED_LIBS, which merely selects __declspec(dllexport)
# on Windows; PyTorch wheels are shared builds, so we define it.
# Providing it via the COMPAT dir (already on CPATH) keeps the upstream sources
# untouched. Real device calls in the hipified sources still go through the HIP
# masquerading APIs; the c10/cuda headers only contribute declarations.
...
#pragma once
// NVDR_ROCM_C10_CMAKE_MACROS_STUB
// Stand-in for the CMake-generated header that PyTorch's ROCm wheels omit.
