...
# make_nvdiffrast_rocm72_gfx1201_reinstall_bundle_v4_v2.sh
# --------------------------------------------------
# Creates a local reinstall bundle for nvdiffrast on:
#   ROCm 7.2 / gfx1201 / PyTorch ROCm / Linux

BUNDLE_DIR="${BUNDLE_DIR:-$HOME/therock_test/nvdiffrast_rocm72_gfx1201_reinstall_bundle_v4}"
...
# --------------------------------------
# Modes:
#   MODE=coverage_probe  -> default; replaces FineRaster with a safe rebaseline probe
...
HIP_IMPL="csrc/common/hipraster/impl"
CUDA_IMPL="csrc/common/cudaraster/impl"

if [[ ! -d "$HIP_IMPL" ]]; then
  echo "FEHLER: $HIP_IMPL fehlt. Der Tree ist nicht ROCm-strukturiert."
  exit 1
fi
...
# nvdiffrast ROCm 7.2 / gfx1201 reinstall bundle

This bundle rebuilds the existing ROCm-patched `~/therock_test/nvdiffrast` tree reproducibly.
...
echo "OK: v4 bundle created at $BUNDLE_DIR"
echo
echo "Run:"
echo "  cd $BUNDLE_DIR"
